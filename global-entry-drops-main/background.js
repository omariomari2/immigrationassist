...
const broadcastStatus = (senderTabId = null) => {
	chrome.storage.local.get(null, (items) => {
		...
		const send = (tabId) => {
			chrome.tabs.sendMessage(tabId, statusMsg)
				.then(() => console.log(`[GED-BG] Sent status to tab ${tabId}`))
				.catch((e) => console.log(`[GED-BG] Failed to send to tab ${tabId}: ${e.message}`));
		};
		...
		// Ensure we send to all other tabs matching our pattern
		chrome.tabs.query({ url: ["http://localhost:5500/*", "http://127.0.0.1:5500/*", "https://localhost:5500/*", "http://0.0.0.0:5500/*"] }, (tabs) => {
			for (let tab of tabs) {
